...
const Bus = ({match}) => {
    ...
    useEffect(function effectFunction() {
        axios.get('http://localhost:5000/bus/all', {headers: { "auth": localStorage.getItem(localStorageAuthKey) } })
            .then(response => {
       ...
       const handleSubmit = e => {
            ...
            e.preventDefault();
            axios
              .delete("http://localhost:5000/bus/delete/" + match.params.id, {headers: { "auth": localStorage.getItem(localStorageAuthKey) } })
              .then(response => {
                console.log(response);
